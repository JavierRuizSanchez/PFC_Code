...
class ver_pedidos_tienda : Fragment() {

    private lateinit var recyclerView: RecyclerView
    private lateinit var pedidoAdapter: PedidosTiendaAdapter
    private lateinit var btnEliminarTienda: Button
    private lateinit var btnEliminarSeleccionadoTienda: Button

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater.inflate(R.layout.fragment_ver_pedidos_tienda, container, false)

        recyclerView = view.findViewById(R.id.recyclerViewPedidosTienda)
        recyclerView.layoutManager = LinearLayoutManager(requireContext())
        pedidoAdapter = PedidosTiendaAdapter()
        recyclerView.adapter = pedidoAdapter
        btnEliminarTienda = view.findViewById(R.id.btn_eliminarPedTienda)
        btnEliminarSeleccionadoTienda = view.findViewById(R.id.btn_eliminarPedidoSeleccionadoTienda)
        val btnInicio: Button = view.findViewById(R.id.btnMenuReservaRes)

        btnInicio.setOnClickListener {
            findNavController().navigate(R.id.action_ver_pedidos_tienda_to_establecimientos)
        }

        obtenerPedidosTienda(2) // Cambia el 2 por el idEstablecimiento correcto
        btnEliminarTienda.setOnClickListener {
            mostrarDialogoAviso() // Cambia el 2 por el idEstablecimiento correcto
        }

        btnEliminarSeleccionadoTienda.setOnClickListener {
    ...
    private fun mostrarDialogoAviso() {
        ...
        builder.setMessage("¡Aviso! Esta acción eliminará todos los pedidos.")
            .setPositiveButton("Aceptar") { dialog, _ ->
                // Acciones al hacer clic en Aceptar
                dialog.dismiss()
                eliminarPedidosTienda(2) // Cambia el 2 por el idEstablecimiento correcto
            }
    ...
    private fun mostrarDialogoAvisoSel() {
        ...
        builder.setMessage("¡Aviso! Esta acción eliminará el producto seleccionado.")
            .setPositiveButton("Aceptar") { dialog, _ ->
                // Acciones al hacer clic en Aceptar
                dialog.dismiss()
                eliminarPedidoSeleccionado(77) // Cambia el 2 por el idEstablecimiento correcto
            }
            .setNegativeButton("Cancelar") { dialog, _ ->
                // Acciones al hacer clic en Cancelar
                dialog.dismiss()
    ...
    private fun eliminarPedidosTienda(idEstablecimiento: Int) {
        val apiService = ApiClient.apiService
        ...
        deletePedidosCall.enqueue(object : Callback<ApiResponse> {
            override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                if (response.isSuccessful) {
                    ...
                    // Actualiza la lista de pedidos después de la eliminación
                    obtenerPedidosTienda(idEstablecimiento)
                } else {
                    // Manejar respuesta de error
    ...
    private fun obtenerPedidosTienda(idEstablecimiento: Int) {
